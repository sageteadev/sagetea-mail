...
bool detectStandardFolder(const QMailAccountId &accountId, StandardFolderInfo standardFolderInfo)
{
    ...
    qDebug() << "[detectStandardFolders] >> Detecting..";

    qDebug() << "[detectStandardFolders] >> Search Paths: " << paths;
    ...
    if (!paths.isEmpty()) {
        ...
        qDebug() << "[detectStandardFolders] >> No exact matches found";
        if (folders.isEmpty()) {
            qDebug() << "[detectStandardFolders] >> Trying OR'd paths";
            ...
            if (folders.isEmpty()) {
                qDebug() << "[detectStandardFolders] >> Still none found. Iterating folders for the account now";
                qDebug() << "Last attempt to find folder";
                auto list = QMailStore::instance()->queryFolders(accountKey);
                foreach(const QMailFolderId &id, list) {
                    QMailFolder folder(id);
                    QString name = folder.displayName().toUpper();
                    switch(standardFolderInfo._standardFolder) {
                    case QMailFolder::InboxFolder:
                    {
                        qDebug() << "Looking for INBOX this folder is: " << name;
                        if (name == "INBOX") {
                            qDebug() << "We FOUND IT!!";
                            folders << id;
                        }
    ...
    if (!folders.isEmpty()) {
        ...
        if (folderId.isValid()) {
            qDebug() << "Setting folder: " << QMailFolder(folderId).displayName();
            QMailFolder folder(folderId);
            folder.setStatus(flag,true);

            // Don't allow deleting INBOX
            if (standardFolder == QMailFolder::InboxFolder) {
...
bool QMail::mkLockDir()
{
    QDir ld(lockDir());
    if (!ld.exists()) {
        return QDir().mkpath(lockDir());
    }
    return true;
}
