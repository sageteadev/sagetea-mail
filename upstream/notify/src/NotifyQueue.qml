...
QtObject {
    ...
    property QtObject __internal: QtObject {
        ...
        property Timer timer: Timer {
            ...
            interval: 50
            onTriggered: {
                console.log("[sageteamail-notify] >> Checking notification queue")
                if (!d.notificationVisible && d.notifications.length) {
                    console.log("[sageteamail-notify] >> Notification pending. Showing now!")
                    d.show(d.notifications.shift(), d.callers.shift(), d.properties.shift())
                } else if (d.notificationVisible) {
                    console.log("[sageteamail-notify] >> Notification already on screen, waiting for completion")
                } else {
                    console.log("[sageteamail-notify] >> No notifications to show")
                }
            }
        }
        ...
        function handleClosed() {
            console.log("[sageteamail-notify] >> Notification complete. Destroying....")
            d.notificationVisible = false
            d.currentNotification.destroy()
            console.log("[sageteamail-notify] >> Notification destroyed.")
            timer.start()
        }
    }
}
