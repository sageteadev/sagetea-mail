...
Item {
    ...
    function send(title, body, props) {
        if (typeof body === "object" || body instanceof Object) {
            console.log("[sageteamail-notify] >> Properties passed as body argument. Correcting....")
            props = body
            body = ""
        }
        if (props) {
            console.log("[sageteamail-notify] >> Queueing custom notification")
            sendCustom(title, body, props)
        } else if (body) {
            if (typeof body === "string" || body instanceof String) {
                console.log("[sageteamail-notify] >> Queueing subtitles notification")
                sendStandard(title, body)
            } else {
                console.error("[sageteamail-notify] >> Invalid body type.")
            }
        } else {
            console.log("[sageteamail-notify] >> Queueing simple notification")
            sendStandard(title, "")
        }
    }
